...
**mm_config — a stored dyno config in stock VESC Tool.** A custom-config page
(no forked GUI) holds the DC-link capacitance `C_dc` — from which the firmware
derives the clamp voltage-loop gains so the response stays invariant across bus
banks — and an optional boot auto-arm block for the bus clamp. Persisted in the
emulated EEPROM; the descriptor + config signature are generated by
`documentation/mm_config_xml_gen.py`. See the mm_config section of the feature
doc.
